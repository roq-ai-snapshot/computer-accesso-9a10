...
import { useWishlistFindFirst, useRoqClient } from 'lib/roq';
...
function WishlistViewPage() {
  ...
  const queryParams = useMemo<{ include: { user: boolean; product: boolean } }>(
    () =>
  ...
  const { data, error, isLoading } = useWishlistFindFirst(queryParams, {}, { disabled: !id });
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('wishlist', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/wishlists/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                <FormListItem
                  label="Added Date"
                  text={data?.added_date ? format(data?.added_date, 'dd-MM-yyyy') : ''}
                />
